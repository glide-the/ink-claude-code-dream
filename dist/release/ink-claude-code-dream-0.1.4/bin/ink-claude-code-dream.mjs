...
async function main() {
  ...
  if (command === "--runtime-manifest") {
    ...
    const { readManifestEnvelope } = await import("../lib/manifest-EVRFHGTD.mjs");
...
`);
  ...
  if (command === "--runtime-doctor") {
    ...
    const { doctor } = await import("../lib/launcher-2JTYEW5O.mjs");
...
`);
  ...
  const { conventionalSignalExitCode, launchOfficialCli } = await import("../lib/launcher-2JTYEW5O.mjs");
