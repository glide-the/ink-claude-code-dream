...
async function main() {
  ...
  if (command === "--runtime-manifest") {
    ...
    const { readManifestEnvelope } = await import("../lib/manifest-BLGU5KQJ.mjs");
...
`);
  ...
  if (command === "--runtime-doctor") {
    ...
    const { doctor } = await import("../lib/launcher-NH3H26QL.mjs");
...
`);
  ...
  const { conventionalSignalExitCode, launchOfficialCli } = await import("../lib/launcher-NH3H26QL.mjs");
