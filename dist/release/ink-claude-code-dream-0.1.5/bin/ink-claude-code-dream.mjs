...
async function main() {
  ...
  if (command === "--runtime-manifest") {
    ...
    const { readManifestEnvelope } = await import("../lib/manifest-Q2FNAZSP.mjs");
...
`);
  ...
  if (command === "--runtime-doctor") {
    ...
    const { doctor } = await import("../lib/launcher-4ER4MK6U.mjs");
...
`);
  ...
  const { conventionalSignalExitCode, launchOfficialCli } = await import("../lib/launcher-4ER4MK6U.mjs");
