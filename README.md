<!-- [Input] Exact checked restored-source snapshot, clean-room Runtime tooling, capability policies, and verification evidence. -->
...
<!-- [Sync] 2026-09-13: restore the complete original source tree, correct research/package licensing, and close 0.1.7 release gates. -->
...
This private repository preserves the original `claude-code-sourcemap/restored-src/src` research layout and builds the separate Dream-compatible Runtime. The checked [`restored-src/`](restored-src/README.md) snapshot contains 1,902 byte-exact files across 35 original module directories, from source commit `a8a678cb6244e6770e1e421767ff0987a1d95549`. It is not a redesigned `src` tree and is not MIT-licensed. The private mixed-source root package is `UNLICENSED`; the independent `src/cleanroom/` implementation and `package/` selector retain their own MIT boundary. Generated core/package files remain Git-ignored. This is technical provenance, not publication authorization.

The historically qualified Linux x64 local-core was built and qualified in a privileged Docker harness from source digest `470ca57d6390e2f9df5e2bb0a6f32b8b62c64f262ae3d02a31349488a08c228e`; its bundle SHA-256 is `a5827e0e6a1f5c3f09f5c4ceca66122893e531d3f31a644811273bae89487e9a`. SDK/Bash, stdio/HTTP MCP, MCP management, OAuth, static contracts, compatibility tests, and lint all exited 0. The qualified package has 64 files, 63 checksum entries, artifact-tree SHA-256 `b69f165f21a333489f35d1fb6e4e55c03b41a61fd11746e75163422505bbcb77`, and `productionEligible=true`. Source provenance remains `2.1.88`; `2.1.241` is only the separately qualified Dream-facing CLI compatibility value. The current profile restores the Linux-only disk path that the 2.1.88 sandbox code searches: `chunks/vendor/seccomp/<arch>/apply-seccomp` is the Docker-style passthrough with SHA-256 `bd2923ee44c624e03bac9efb57c84d72419726783ac7557acb708e431c16d74d`, and `unix-block.bpf` is checksum-pinned from `@anthropic-ai/sandbox-runtime@0.0.45`.

This is a technical artifact decision, not a publication or deployment grant: `productionEligible=true`, while `publicationAllowed=false` and `redistributionAllowed=false`. The Dream manifest gate and backend suite passed (1,954 passed, 24 skipped, 607 subtests), the historical standard Runtime run passed 44 tests with 2 external OAuth-fixture tests skipped, and the historical custom-core real Dream main journey passed. The official MCP Python SDK `2.0.0` fixture at commit `6f69a3758ebf2ee55ce050f58b470ce11af71133` separately passed the complete OAuth CLI contract 3/3 through pipe and real-PTY lanes. The final real Comfy acceptance also passed through Dream's public configure/auth/callback/inventory/logout/remove path: `comfyui-cloud` `0.40.1` connected with 41 tools and a complete 16-stage safe receipt ending in `credentials_present` → `flow_resolved` → `success_stdout_flushed`.
...
Runtime `0.1.7` restores the reference artifact's physical package boundary: the repository root is again only the private build workspace, while [`package/`](package/) owns the selector package's `package.json`, package-root `cli.js`, README, and license. The selector intentionally has no `exports`; both `claude` and Dream's explicit `ink-claude-code-dream` alias resolve to `cli.js`, which selects one exact native optional package. This preserves Dream's four-target standalone distribution without pretending that generated staging directories are source packages.

Runtime `0.1.7` now restores the complete `restored-src/src` subtree as well as the package-root selector shape. The incomplete, unpublished `0.1.6` candidate is superseded. The [source/package alignment decision](docs/design/claude-sourcemap-package-contract-alignment.md) records every original module directory, exact subtree identity, byte-inventory gate, and research/publication boundary. The restored tree is immutable and excluded from clean-room compilation, npm tarballs, and SBOM provenance. The prior removal of invented CLI behavior is retained; no ZIP policy, Anthropic package identity, or unsupported Ink/IDE/updater claim is added. All `0.1.7` production, redistribution, publication, and target-qualification gates are closed. The immutable `0.1.5` receipt cannot authorize it.
...
Git now contains the exact research-only `restored-src/src` snapshot alongside replayable repository-authored tooling. `node scripts/sync-restored-source.mjs verify` binds its 1,902-file subtree and original bytes; lint invokes it automatically. The reference sourcemap checkout remains read-only. No Anthropic redistribution authorization has been obtained, so restored source and derived artifacts must not be publicly published or redistributed. The clean-room package remains restored-source-free and MIT; the mixed-source root is `UNLICENSED`. Generated artifacts, credentials, complete environment data, transcripts, Workspace content, and materialized plugins remain excluded from Git.

The historical exact-Node verification command, `PATH=/Users/dmeck/.nvm/versions/node/v24.13.0/bin:$PATH bun run verify`, exited 0: Node 44 passed with 2 external OAuth-fixture tests skipped, MCP compatibility 46 passed with 6 authorized-source fixtures skipped, and the SDK contract, acceptance, release verification, and archive reproducibility all passed. A separate authorized-source replay, `INK_AUTHORIZED_CORE_SOURCE_ROOT=/Users/dmeck/project/claude-code-sourcemap/restored-src bun --cwd compat/mcp-auth test`, then executed those six fixtures and passed 52/52 with zero failures. The historical archive SHA-256 is `64c919d1f11b2770497a080c4cdeb8587925f45d928912459b31647e1b68eb38`; checksum-inventory SHA-256 is `61e12c7c1828c05fb6e70535abb36ff1fbe924aaba2d78787b9ce8e832b3947c`. Current candidate validation is recorded separately in [Runtime 0.1.7 notes](docs/build/runtime-0.1.7-release-notes.md).
...
`0.1.5` 的 digest-bound Dream 验收仍作为不可变历史文件保存。未发布的 `0.1.6` 已被 `0.1.7` 完整源码结构修复候选取代。仓库恢复了原始 35 个模块目录和 1,902 个文件，而不是重新设计目录；恢复源码只作为研究快照，不进入 clean-room npm 制品。`0.1.7` 尚无同 SHA 真实业务验收和四目标资格回执，checked policy 仍明确设置 `productionEligible=false`、`publicationAllowed=false`、`redistributionAllowed=false`、`npmPublishAllowed=false`。本地 fixture 的技术验证不会解除发布 gate。
