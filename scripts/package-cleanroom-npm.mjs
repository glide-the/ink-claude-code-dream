...
const sourcePackageRoot = path.join(repositoryRoot, policy.metaPackage.sourceRoot);
const sourcePackage = JSON.parse(await readFile(path.join(sourcePackageRoot, "package.json"), "utf8"));
const sourceSelector = await readFile(path.join(sourcePackageRoot, "cli.js"));
const sourcePackageReadme = await readFile(path.join(sourcePackageRoot, "README.md"));
const sourcePackageLicense = await readFile(path.join(sourcePackageRoot, "LICENSE.md"));
...
function validatePolicy() {
  ...
  if (
    ...
    rootPackage.name !== "ink-claude-code-dream" ||
    rootPackage.private !== true ||
    Object.hasOwn(rootPackage, "bin") ||
    policy.metaPackage.sourceRoot !== "package" ||
    sourcePackage.name !== policy.metaPackage.name ||
    sourcePackage.version !== policy.version ||
    sourcePackage.private !== true ||
    sourcePackage.type !== "module" ||
    Object.hasOwn(sourcePackage, "exports") ||
    !sourcePackageLicense.equals(licenseBody) ||
    policy.license !== "MIT" ||
    sourcePackage.license !== "MIT" ||
    JSON.stringify(Object.keys(sourcePackage.bin ?? {}).sort()) !==
      JSON.stringify([...policy.metaPackage.commands].sort()) ||
    Object.values(sourcePackage.bin ?? {}).some(entrypoint => entrypoint !== policy.metaPackage.entrypoint) ||
    JSON.stringify(stableValue(sourcePackage.optionalDependencies)) !==
      JSON.stringify(stableValue(expectedOptionalDependencies)) ||
