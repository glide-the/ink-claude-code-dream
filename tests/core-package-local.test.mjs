...
// [Sync] 2026-09-15: require local-core Runtime 0.1.10 and reuse an explicitly selected Dream Python for isolated worktree verification.
...
const artifactId = "ink-claude-code-dream-0.1.10";
...
async function runDreamManifestGate(executable) {
  ...
  const program = [
    ...
    "from libs.claude_agent_kit.server.sdk_env import DREAM_CLAUDE_CLI_VERSION, require_dream_claude_runtime_manifest",
    "from claude_mcp.identity import _cli_has_secure_storage_marker",
    "executable = Path(sys.argv[2])",
    "print(json.dumps({'expectedVersion': DREAM_CLAUDE_CLI_VERSION}, sort_keys=True), flush=True)",
...
test("Dream's real production manifest gate rejects an unqualified artifact and accepts a fully qualified one", async t => {
  ...
  try {
    ...
    const dreamReceipt = JSON.parse(accepted.stdout.trim().split("\n").at(-1));
    assert.equal(
