...
// [Output] Prove identical directories/files/modules/bytes and actual src-based compilation, not a side snapshot.
...
test("src has all original module directories, file paths, modes and exact bytes", async () => {
 const receipt = await json("restored-src/source-snapshot.json");
 ...
 const run = spawnSync(process.execPath, ["scripts/sync-restored-source.mjs", "verify"], { cwd: root, encoding: "utf8" });
 assert.equal(run.status, 0, run.stderr);
 const verified = JSON.parse(run.stdout);
 assert.equal(verified.sameModules, true);
...
test("the actual default build uses original project modules as one Runtime", async () => {
 ...
 assert.equal(layout.publicationAllowed, false);
 assert.equal(layout.redistributionAllowed, false);
});
