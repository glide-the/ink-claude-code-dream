...
// [Sync] 2026-09-13: require Runtime 0.1.7 release identity and paths.
...
test("Runtime manifest diagnostic does not need or launch a Claude core", async () => {
  ...
  assert.equal(envelope.manifest.runtime.integration.sdkVersion, "0.2.145");
  assert.equal(envelope.manifest.runtime.integration.dreamObservedSdkVersion, "0.2.145");
...
test("pruning decision never converts outer omissions into core deletion", async () => {
  const decision = JSON.parse(
    await readFile(
      resolve("dist/release/ink-claude-code-dream-0.1.7/manifest/pruning-decision.json"),
      "utf8",
    ),
  );
...
test("esbuild keeps launcher and Runtime manifest diagnostic behind dynamic imports", async () => {
  const metafile = JSON.parse(
    await readFile(
      resolve(
        "dist/release/ink-claude-code-dream-0.1.7/manifest/esbuild-metafile.json",
      ),
      "utf8",
    ),
  );
